...
function bool IsHeadShot(vector HitLoc, vector Ray, float AdditionalScale)
{
    ...
    bBlockedHS = false;
    bIsBlocking = bServerBlock && !IsTweening(1);
    if ( bIsBlocking ) {
        AdditionalScale += BlockAddScale;
    }
    else {
        // XXX: why do we need +1.0 here? Can we raise HeadScale instead?
        AdditionalScale += 1.0;
    }

    bHeadShot = class'ScrnZedFunc'.static.IsHeadShot(self, HitLoc, ray, AdditionalScale, vect(0,0,0));
...
Ignores StartCharging;
    ...
    function EndState()
    {
        ...
        BruteController(Controller).RageFrustrationTimer = 0;
